...
def prepare_avd(device: str, avd_name: str):
    ...
    creation_cmd = 'avdmanager create avd -f -n {name} -b {img_type}/{sys_img} -k "system-images;android-{api_lvl};' \
                   '{img_type};{sys_img}" -d {device} -c 6000M -p {path}'.format(name=avd_name, img_type=IMG_TYPE,
                                                                        sys_img=SYS_IMG,
...
def run():
    ...
    if is_first_run:
        cmd = 'emulator/emulator @{name} -gpu swiftshader_indirect -verbose -wipe-data {custom_args}'.format(name=avd_name, custom_args=custom_args)
    else:
        cmd = 'emulator/emulator @{name} -gpu swiftshader_indirect -verbose {custom_args}'.format(name=avd_name, custom_args=custom_args)
